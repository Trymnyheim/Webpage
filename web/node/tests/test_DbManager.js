...
async function test_createTable() {
    await DbManager.dropTable('Tester')
    // Test 1: Creating a table::
    try {
        await DbManager.createTable('Tester');
...
async function test_insertTask() {
    ...
    try {
        await DbManager.insertTask('Tester', new Task(1, 'Test task', "A description", "2024-10-31 10:30:00", 0));
    ...
    try {
        await DbManager.insertTask('Jqosmeisl', new Task(1, 'Test task', "A description", "2024-10-31 10:30:00", 0));
...
async function test_updateTask() {
    ...
    try {
        await DbManager.updateTask('Tester', new Task(1, 'Updated task', "A description", "2024-10-31 10:30:00", 0));
...
async function test_deleteTask() {
    ...
    try {
        await DbManager.deleteTask('Tester', 1);
...
async function test_getAllTasks() {
    ...
    await DbManager.insertTask('Tester', new Task(1, 'Test task 1', "A description", "2024-10-31 10:30:00", 0));
    await DbManager.insertTask('Tester', new Task(2, 'Test task 2', "A description", "2024-10-31 10:30:00", 0));
    try {
        const tasks = await DbManager.getAllTasks('Tester');
        if (tasks.length === 2) {
    ...
    await DbManager.insertTask('Tester', new Task(3, 'Test task 3', "A description", "2024-10-31 10:30:00", 0));
    try {
        const tasks = await DbManager.getAllTasks('Tester');
        if (tasks.length === 3) {
    ...
    try {
        const tasks = await DbManager.getAllTasks('ertjasokf');
